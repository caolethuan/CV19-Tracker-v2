...
loadSummary = async (country) => {
    ...
    if (!isGlobal(country)) {
        summary = summaryData.filter(e => {
            e["Country_text"] === country
            return e
        })
    }
    showConfirmedTotal(summary["Total Cases_text"])
    showRecoveredTotal(summary["Total Recovered_text"])
    showDeathsTotal(summary["Total Deaths_text"])
...
loadCountrySelectList = async () => {
    ...
    let country_list=[]
    summaryData.forEach( e => countries_list.push(e["Country_text"]))
